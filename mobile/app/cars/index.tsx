...
export default function CarsListScreen() {
  ...
  const [filters, setFilters] = useState({ make: '', purpose: '' });
    ...
    <View style={styles.container}>
      ...
      <View style={styles.filterSection}>
        <View style={styles.searchBar}>
          ...
          <TextInput 
            ...
            value={filters.make}
            onChangeText={(make) => setFilters(prev => ({ ...prev, make }))}
          />
        </View>
          ...
          {['', 'rent', 'buy'].map((p) => (
            ...
            >
              <Text style={[styles.chipText, filters.purpose === p && styles.chipTextActive]}>
                {p ? `FOR ${p.toUpperCase()}` : 'ALL VEHICLES'}
              </Text>
            </TouchableOpacity>
          ))}
        </ScrollView>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <EmptyState title="No vehicles found" subtitle="Try adjusting your filters" />
          }
        />
      )}
    </View>
