  ...
  return (
    <div className="space-y-10">
      ...
      <div className="bg-white rounded-[2.5rem] shadow-xl border border-gray-100 overflow-hidden">
        <div className="overflow-x-auto"><table className="w-full text-left border-collapse"><thead className="bg-gray-50/50 border-b border-gray-100"><tr><th className="p-6 text-[10px] font-black uppercase text-gray-400 tracking-widest">Preview</th><th className="p-6 text-[10px] font-black uppercase text-gray-400 tracking-widest">Property Info</th><th className="p-6 text-[10px] font-black uppercase text-gray-400 tracking-widest">Purpose</th><th className="p-6 text-[10px] font-black uppercase text-gray-400 tracking-widest">Price</th><th className="p-6 text-[10px] font-black uppercase text-gray-400 tracking-widest">Status</th><th className="p-6 text-center text-[10px] font-black uppercase text-gray-400 tracking-widest">Actions</th></tr></thead>
            <tbody className="divide-y divide-gray-50">{houses.map((h) => { const images = parseImages(h.images); const imageUrl = images[0] ? `${SERVER_BASE_URL}${images[0]}` : 'https://via.placeholder.com/150x100?text=No+Image'; const price = h.monthly_rent_price || h.purchase_price; return (<tr key={h.id} className="hover:bg-gray-50/50 group transition-colors"><td className="p-6"><div className="w-20 h-14 rounded-xl overflow-hidden border shadow-sm"><img src={imageUrl} className="w-full h-full object-cover" /></div></td><td className="p-6"><p className="font-bold text-primary-dark text-xs uppercase tracking-tight">{h.title}</p><p className="text-[10px] text-gray-400 font-bold uppercase">{h.district}, {h.province}</p></td><td className="p-6"><span className={`text-[10px] font-black uppercase ${h.purpose === 'sale' ? 'text-purple-500' : 'text-blue-500'}`}>{h.purpose || 'RENT'}</span></td><td className="p-6 font-bold text-primary-dark text-xs whitespace-nowrap">Rwf {Number(price).toLocaleString()}</td><td className="p-6"><span className={`px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest ${h.status === 'available' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>{h.status}</span></td><td className="p-6"><div className="flex justify-center gap-2"><button onClick={() => handleEdit(h)} className="p-3 bg-gray-50 text-blue-600 hover:bg-blue-600 hover:text-white rounded-xl transition-all"><Edit2 size={16} /></button><button onClick={() => handleDelete(h.id)} className="p-3 bg-gray-50 text-red-600 hover:bg-red-600 hover:text-white rounded-xl transition-all"><Trash2 size={16} /></button></div></td></tr>);})}</tbody></table></div>
      </div>
    </div>
  );
};
